...
ACTIVE is used to perform deployment and testing of facility management research codes. The general use case involves wrapping business logic in ACTIVE Component subclasses, then creating one or more Environment files which define how those Components are to be deployed. Each Environment may describe one scenario in which a single code is to be deployed (such as local simulation versus field validation) and multiple Environments can be used together to handle different goals for the facility. These are the two primary concepts within ACTIVE's architecture:

- **Components** are code which handle particular parts of deployment, from communicating with external code or devices (Controllers, Data Stores, and Multiplexers), defining business logic (Strategies), to instantiating the system which will run the code and injecting the code into it (Agents).
...
Above: An example use case. We have two types of control code to run (ASHRAE 36 guidelines and a machine learning approach). We also have two deployment scenarios: testing the algorithm on a local EnergyPlus Simulation and a field validation experiment using a real building. Using ACTIVE, we can prepare two Environment files, one each with the appropriate Controllers to communicate with the simulation or physical devices. Likewise, the two control algorithms are implemented in two Strategy Components. Both algorithms can be run in both simulation and experiment modes easily by swapping out which Strategy is defined to be run by each Environment, allowing a greatly simplified loop from algorithm development in simulation to field validation and back. Note that the simulated RTU in EnergyPlus (and the VAVs which are treated as sub-devices) provides more data than does the real one, requiring the use of a Multiplexer Component that ties it with a data logger device that provides the missing data to fully fufill the interface. 
