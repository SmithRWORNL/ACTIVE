...
The Automated Control Testbed for Integration, Verification, and Emulation (ACTIVE) framework is a software platform designed to support the optimized operation and management of a wide range of building types. It enables the development, testing, and validation of diverse control strategies, including AI-based, rule-based, and model-based approaches. The platform facilitates a seamless transition from simulation-based evaluation of control strategies to real-world field validation and deployment.

ACTIVE supports the full building management lifecycle, encompassing data acquisition and management, system monitoring, optimized control, adaptive learning services, device dispatch and coordination, as well as advanced analytics and visualization. Together, these capabilities provide an integrated environment for improving building performance, operational efficiency, reducing energy cost, and reliability.

These are the two primary concepts within ACTIVE's architecture:

- **Components** are code which handle particular parts of deployment, from communicating with external code or devices (Controllers, Data Stores, and Multiplexers), defining business logic (Strategies), instantiating the system which will run the code and injecting the code into it (Agents), to reproducing the functionality of a real external API (Emulators).
- An **Environment** defines a deployment scenario in a JSON format file, listing the Components to be used and how they are to be configured. ACTIVE uses the Environment to determine how to integrate the Components with each other.

![Diagram of Strategies and Environments relationship](images/experiment_and_simulation_environments.png)

Above: An example use case. We have three types of control code to run: rule based guidelines, an MPC algorithm, and a machine learning approach. We also have two deployment scenarios: testing the algorithm on a local EnergyPlus Simulation and a field validation experiment using a real building. Using ACTIVE, we can prepare two Environment files, one each with the appropriate Controllers to communicate with the simulation or physical devices. Likewise, the three control algorithms are implemented in three Strategy Components. Both algorithms can be run in both simulation and experiment modes easily by swapping out which Strategy is defined to be run by each Environment, allowing a greatly simplified loop from algorithm development in simulation to field validation and back. Note that the simulated RTU in EnergyPlus (and the VAVs which are treated as sub-devices) provides more data than does the real one, requiring the use of a Multiplexer Component that ties it with a data logger device that provides the missing data to fully fulfill the interface. 

![Diagram of five ACTIVE environments](images/ACTIVE_environments_example.png)
Above: An example of how Environments may be set up in a realistic use case. The local Environment in red launches an EnergyPlus simulation for algorithm development and testing by scientists on their own computers. The experiment Environment in purple and the remote experiment Environment in blue deploy the software for a real field validation experiment, the difference being that the experiment Environment is intended to be run inside the facility's network while the remote experiment Environment utilizes a server that allows remote calls from verified users outside that network. The experiment test Environment (yellow) and remote experiment test Environment (green) integrate the same Controllers and Multiplexers but with different configurations, pointing them to communicate with Emulators which are also launched by the Environment, allowing a communications test before physical deployment, especially in cases where the physical facility is already in use. The different Environments reuse many Components. Of special note is the AI Strategy Component, which contains the entire of the scientific control algorithm. All algorithmic development is centered in that one class which is run agnostic of whether it is being deployed into a simulation or at the physical facility, allowing complete compartmentalization of AI and communication software development. Likewise, the AI Strategy can be swapped out for a different Rule Based Strategy while leaving the rest of the code untouched.

![Diagram of two ACTIVE environments](images/ACTIVE_use_case.png)
Above: An example of a project that used ACTIVE to handle code deployment. Each Environment contains a number of Components, each of which handle a specific part of the integration. Controllers perform calls to remote devices, Data Stores provide read and write access to long term data storage, and Strategies execute control logic. Note that not all the above Components are necessarily contained in ACTIVE, as some may instead be project specific extensions to allow support for additional vendors.
